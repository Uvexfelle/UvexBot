...
//  Spoiltime
export const getSpoilTime = (pseudoRunnereuse, live) => {
    try {
        let res = null;
        const runnerCible = getSrcId(pseudoRunnereuse);

    //  Catégorie via titre
        const liveUid = getTitreUid(live);
        if (liveUid) {
            res = dbBigData.prepare(`
                SELECT r.pb_manual_time, r.pb_src_time, r.pb_rank, r.predicted_rank, r.uid, r.game_id,
                    e.cat_pop, e.lead_json, e.variables_json, w.wr_time
                FROM current_records r
                JOIN src_entities e ON r.uid = e.uid
                JOIN world_records w ON r.uid = w.uid
                WHERE r.runner_id = ? AND r.uid = ?
                LIMIT 1
            `).get(runnerCible, liveUid);
        }

    //  Fallback
        let gId = null;
        if (!res && live?.game) {
            const searchLive = findGameId(live.game);
            if (searchLive) gId = searchLive.id;
        }
        ...
        if (!res && gId) {
            res = dbBigData.prepare(`
                ...
                JOIN src_entities e ON r.uid = e.uid
                JOIN world_records w ON r.uid = w.uid
                WHERE r.runner_id = ? AND r.game_id = ?
        ...
        if (!res && gId) {
            ...
            if (familyIds.length > 0) {
                ...
                res = dbBigData.prepare(`
                    ...
                    JOIN src_entities e ON r.uid = e.uid
                    JOIN world_records w ON r.uid = w.uid
                    WHERE r.runner_id = ? AND r.game_id IN (${placeholders})
                    ORDER BY e.cat_pop DESC, r.pb_date DESC
                    LIMIT 1
                `).get(runnerCible, ...familyIds);
            }
        }
        if (!res) return `Est-ce qu'on est vraiment en speedrun ?? Si c'est le cas je suis dsl je ne peux pas t'aider...`;
        ...
        const myRank = res.pb_rank || total;
        const myPb = res.pb_src_time;
        ...
        //  Réponse
        return `${gameName} ${catName}, la run vas être un .... ${secToTime(finalSec)}`;

    } catch (err) {
        console.error(`[ça pue du cul...] Spoiltime vas pas bien :`, err);
        return `PERDU`;
    }
};